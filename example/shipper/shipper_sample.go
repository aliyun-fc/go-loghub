...
func main() {
	...
	logstoreName := "test"
	prjCfg := util.Project
	logProject, err := sls.NewLogProject(prjCfg.Name, prjCfg.Endpoint, prjCfg.AccessKeyID, prjCfg.AccessKeySecret)
	logstore, err := logProject.GetLogStore(logstoreName)
	if logstore == nil {
		fmt.Printf("GetLogStore fail, err:%v\n", err)
		err = util.Project.CreateLogStore(logstoreName, 1, 2)
		if err != nil {
			panic(err)
		}
